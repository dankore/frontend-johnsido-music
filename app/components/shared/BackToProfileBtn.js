...
function BackToProfileBtn({ username, firstName, linkDomId, linkChild }) {
  return (
    <Link
      ...
      className={
        linkCSS + ' transition duration-500 ease-in-out transform hover:scale-110 flex items-center'
      }
      to={`/profile/${username}`}
    >
      ...
      {linkChild && <span className="block ml-2 text">Back {firstName}&apos;s profile</span>}
    </Link>
  );
}
