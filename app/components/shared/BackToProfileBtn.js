...
function BackToProfileBtn({ username, firstName, linkDomId, linkChild }) {
  return (
    <Link
      ...
      className={
        linkCSS +
        ' inline-block transition duration-500 ease-in-out transform hover:scale-110 flex items-center'
      }
      to={`/profile/${username}`}
    >
      ...
      {linkChild && <span className="text ml-2 block">Back {firstName}&apos;s profile</span>}
    </Link>
  );
}
